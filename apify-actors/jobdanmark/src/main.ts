...
import { createJobdanmarkHandler, searchStartUrls } from "./routes.js";
...
if (mode === "fixture") {
  ...
  // JobDanmark is server-rendered, so a Cheerio crawler over HTTP is enough.
  const crawler = new CheerioCrawler({
    maxConcurrency: 5,
    requestHandlerTimeoutSecs: 120,
    requestHandler: createJobdanmarkHandler({ maxItems }),
  });
...
// Ship the dataset to the Sbotter webhook (?source=jobdanmark) with an HMAC-SHA256
