...
import { createMomentHandler, searchStartUrls } from "./routes.js";
...
if (mode === "fixture") {
  ...
  // The Hub (thehub.io) is a React SPA, so we render with a real browser. The
  // handler paginates and maps listings (see src/routes.ts).
  const crawler = new PlaywrightCrawler({
    headless: true,
    navigationTimeoutSecs: 90,
    requestHandlerTimeoutSecs: 300,
    requestHandler: createMomentHandler({ maxItems }),
  });
...
// Ship the dataset to the Sbotter webhook (?source=moment) with an HMAC-SHA256
