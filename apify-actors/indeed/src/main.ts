...
import { createIndeedHandler, searchStartUrls } from "./routes.js";
...
if (mode === "fixture") {
  ...
  const location = input.location ?? "Danmark";
  const maxItems = input.maxItems ?? 1000;

  // Indeed is heavily JS-rendered and aggressively blocks scrapers, so we use a
  // real browser. The proxy + human-like pacing are configured here; the handler
  // does the extraction (see src/routes.ts). Use a residential proxy at scale.
  ...
  const crawler = new PlaywrightCrawler({
    ...
    requestHandler: createIndeedHandler({ maxItems }),
  });
...
// Ship the dataset to the Sbotter webhook (?source=indeed) with an HMAC-SHA256
